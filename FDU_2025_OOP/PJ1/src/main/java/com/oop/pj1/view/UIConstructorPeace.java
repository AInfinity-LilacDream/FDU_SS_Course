...
public class UIConstructorPeace extends AbstractUIConstructor {

    @Override
    public GridPane createBoard(GameController game) {
        blackPlayer = new Label("黑棋 [Tom]");
        whitePlayer = new Label("白棋 [Jerry]");
        ...
        for (int i = 0; i < BOARD_SIZE; ++i) {
            ...
            for (int j = 0; j < BOARD_SIZE; ++j) {
                ...
                StackPane tile = new StackPane(circle);
    ...
    public VBox createGameInfo(GameController game) {
        ...
        vbox.getChildren().addAll(title, blackPlayer, whitePlayer, demoMode, exitGame);
        changeLabelBorder(game);

        // 设置按钮宽度
        exitGame.setPrefWidth(GlobalEntities.GAME_INFO_WIDTH - 20);
        demoMode.setPrefWidth(GlobalEntities.GAME_INFO_WIDTH - 20);
